...
// ─── Shared styles (defined outside component so they never change reference) ──
...
// ─── Field + Section MUST live outside AdminVictimForm ─────────────────────────
// If defined inside the render function, React treats them as new component types
// on every re-render, unmounting the input and causing the "one character" bug.
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: '1rem' }}>
      <label style={{ display: 'block', fontSize: '.78rem', color: 'var(--muted)', marginBottom: '.35rem' }}>
        {label}
      </label>
      {children}
    </div>
  ...
  return (
    <div style={{ maxWidth: '780px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <button onClick={() => navigate('/admin/victims')}
          style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '8px', padding: '.4rem .8rem', color: 'var(--muted)', fontFamily: 'Cairo, sans-serif', cursor: 'pointer' }}>
          ← رجوع
        </button>
        <h1 style={{ fontSize: '1.3rem', fontWeight: 900 }}>
          {isEdit ? 'تعديل بيانات الضحية' : 'إضافة ضحية جديدة'}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Section title="🪪 المعلومات الأساسية">
          <Field label="الاسم بالعربية *">
            <input value={form.name_ar} onChange={set('name_ar')} required
              placeholder="مثال: محمد أحمد العلي" style={inp} />
          </Field>
          <div style={grid2}>
            <Field label="العمر *">
              {/* type="text" + inputMode lets users type freely without spinner arrows */}
              <input
                type="text" inputMode="numeric" pattern="[0-9]*"
                value={form.age} onChange={set('age')} required
                placeholder="مثال: 35" style={inp}
              />
            </Field>
            <Field label="المهنة / الصفة">
              <input value={form.profession} onChange={set('profession')}
                placeholder="طالب، معلمة، عامل..." style={inp} />
            </Field>
          </div>
        </Section>
        ...
        <Section title="📍 الموقع والتواريخ">
          ...
          <div style={grid2}>
            <Field label="خط العرض (lat)">
              <input type="text" inputMode="decimal" value={form.lat} onChange={set('lat')}
                placeholder="32.517" style={inp} />
            </Field>
            <Field label="خط الطول (lng)">
              <input type="text" inputMode="decimal" value={form.lng} onChange={set('lng')}
                placeholder="35.152" style={inp} />
            </Field>
          </div>
          <div style={grid2}>
            {/* dir="ltr" is required — dates are always left-to-right regardless of page direction */}
            <Field label="تاريخ الميلاد">
              <input type="date" dir="ltr" value={form.date_of_birth} onChange={set('date_of_birth')} style={inp} />
            </Field>
            <Field label="تاريخ الوفاة *">
              <input type="date" dir="ltr" value={form.date_of_death} onChange={set('date_of_death')} required style={inp} />
            </Field>
          </div>
        </Section>
        ...
        <Section title="📝 السيرة الشخصية">
          ...
          {tab === 'ar' && <textarea value={form.bio_ar} onChange={set('bio_ar')} rows={6} placeholder="السيرة الشخصية بالعربية..." style={{ ...inp, resize: 'vertical' }} />}
          {tab === 'he' && <textarea value={form.bio_he} onChange={set('bio_he')} rows={6} dir="rtl"  placeholder="ביוגרפיה בעברית..."       style={{ ...inp, resize: 'vertical' }} />}
          {tab === 'en' && <textarea value={form.bio_en} onChange={set('bio_en')} rows={6} dir="ltr"  placeholder="Biography in English..."   style={{ ...inp, resize: 'vertical' }} />}
        </Section>
        ...
        <Section title="🌐 الاسم بلغات أخرى (اختياري)">
          <div style={grid2}>
            <Field label="الاسم بالعبرية">
              <input value={form.name_he} onChange={set('name_he')} dir="rtl" placeholder="שם בעברית" style={inp} />
            </Field>
            <Field label="الاسم بالإنجليزية">
              <input value={form.name_en} onChange={set('name_en')} dir="ltr" placeholder="Name in English" style={inp} />
            </Field>
          </div>
        </Section>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
          <button type="button" onClick={() => navigate('/admin/victims')}
            style={{ padding: '.75rem 1.5rem', border: '1px solid var(--border)', borderRadius: '10px', background: 'none', color: 'var(--muted)', fontFamily: 'Cairo, sans-serif', cursor: 'pointer' }}>
            إلغاء
          </button>
          <button type="submit" disabled={saving}
            style={{ padding: '.75rem 1.75rem', border: 'none', borderRadius: '10px', background: saving ? 'rgba(201,168,76,.5)' : 'var(--gold)', color: '#0d1117', fontFamily: 'Cairo, sans-serif', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontSize: '1rem' }}>
